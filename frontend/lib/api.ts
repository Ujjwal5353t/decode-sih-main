...
async function fetchApi<T>(
  ...
  try {
    ...
    if (err.name === "AbortError") {
      const error = new Error("Server request timed out. The remote database or backend server is taking longer than usual to respond. Please try again.") as ApiError;
      error.status = 504;
      throw error;
    }
